...
# --- 6. PAQUETES ESENCIALES (SOLO CLI) --- 
  ...
  environment.shellAliases = {
    nix-up = "pushd ~/nixos > /dev/null && echo '--- 🔄 Actualizando ---' && nix flake update && echo '--- 🏗️ Construyendo ---' && sudo nixos-rebuild build --flake .#${config.networking.hostName} && echo '--- 📋 Diferencias ---' && nvd diff /run/current-system result && echo '--- 🚀 Aplicando ---' && sudo nixos-rebuild switch --flake .#${config.networking.hostName} && popd > /dev/null";
